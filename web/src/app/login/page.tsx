...
const FEATURES = [
  { icon: "✦", text: "Capture raw thoughts — voice, text, photos" },
...
export default async function LoginPage({
  ...
  return (
    <div className="login-root">
      ...
      <a href="#login-main" className="login-skip-link">Skip to sign-in form</a>

      {/* Brand panel — left on desktop, top on mobile */}
      <aside className="login-panel-brand" aria-label="About DayPage">
        <div className="login-panel-brand__inner">
          <div className="login-panel-brand__mark">
            <svg
              width="32"
              height="32"
              viewBox="0 0 36 36"
              fill="none"
              aria-hidden="true"
              className="login-panel-brand__logo"
            >
              <rect x="1" y="1" width="34" height="34" rx="10" fill="rgba(255,255,255,0.15)" />
              <path d="M18 8 L28 18 L18 28 L8 18 Z" fill="white" opacity="0.9" />
              <circle cx="18" cy="18" r="4" fill="rgba(255,255,255,0.3)" />
            </svg>
            <span className="login-panel-brand__wordmark">DAYPAGE</span>
            <span className="login-panel-brand__badge">private beta</span>
          </div>

          <div className="login-panel-brand__content">
            <h1 className="login-panel-brand__title">
              Your days,<br />
              <em className="login-panel-brand__accent">distilled.</em>
            </h1>
            <p className="login-panel-brand__sub">
              Dump raw thoughts throughout the day. DayPage compiles them into a
              structured diary entry and a growing knowledge graph — automatically.
            </p>

            <ul className="login-panel-features" aria-label="Key features">
              {FEATURES.map((f) => (
                <li key={f.icon} className="login-panel-feature">
                  <span className="login-panel-feature__icon" aria-hidden="true">{f.icon}</span>
                  <span className="login-panel-feature__text">{f.text}</span>
                </li>
              ))}
            </ul>
          </div>
        </div>
        <div className="login-panel-brand__grid" aria-hidden="true" />
      </aside>

      {/* Auth panel — right on desktop, below on mobile */}
      <main id="login-main" className="login-panel-auth">
        <div className="login-card landing-fade-in--card">
          <div className="login-brand">
            ...
            <div>
              <h2 id="login-heading" className="login-brand__name">
                Sign in to DayPage
              </h2>
